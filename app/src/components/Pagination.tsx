...
export function Pagination({ page, pageSize, total, onPageChange, onPageSizeChange }: Props) {
  ...
  const btn =
    'rounded-md border border-slate-200 dark:border-slate-700 px-2.5 py-1 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40'

  return (
    <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-2.5">
      <p className="text-sm text-slate-600 dark:text-slate-300">
        {total === 0 ? 'No results' : (
          <>
            <span className="font-medium text-slate-900 dark:text-slate-100">{from.toLocaleString()}–{to.toLocaleString()}</span>
            {' of '}
            <span className="font-medium text-slate-900 dark:text-slate-100">{total.toLocaleString()}</span>
          </>
        )}
      </p>
